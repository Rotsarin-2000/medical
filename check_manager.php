...
include './inserview/check_manager.php';
...
$employee_name = $response_chk['department_name'] . "_" . $response_chk['department_name'];
$employee_id = $response_chk['employee_id'];
$department = $response_chk['department_name'];
...
// //         WHERE status = 0 
// //         AND JSON_UNQUOTE(JSON_EXTRACT(medical, '$.department')) = ? 
// //         ORDER BY id DESC";
...
// สำหรับดึงข้อมูลจากฐานข้อมูล
function fetchMedicalData($conn, $table, $department, $year)
{
...
// ดึงข้อมูลจาก medical_creates  จากตาราง medical_creates *ตาราง data ที่มีการบันทึกรายการเบิกค่ารักษา
$result = fetchMedicalData($conn, 'medical_creates', $department, $year);

// กรณีที่ไม่มีข้อมูลในตาราง medical_creates ให้ดึงข้อมูลจากตาราง m_creates มาแสดง 
$result_medical_create = fetchMedicalData($conn, 'm_creates', $department, $year);

// if ($result->num_rows === 0 && $result_medical_create->num_rows === 0) {
//     echo "No medical data found.";
// }

// สิทธิ์ user ที่ใช้งานหน้านี้
...
<body class="sidebar-mini sidebar-closed sidebar-collapse">
    <div class="wrapper">
        ...
        <div class="content-wrapper" style="min-height: 2080.26px;">
            <div class="container-fluid">
                <div class="col-md-12 p-2" id="dataTableContainer">
                    <div class="card">
                        <div class="card-header">
                            <i class="fas fa-list-ul" style="font-size: 22px;"></i> <span>
                                List by department</span>
                        ...
                        <div class="card-body">
                            <div class="table table-responsive-md">
                                        ...
                                        <tr>
                                            ...
                                            if ($result && $result->num_rows > 0) {
                                                while ($row = $result->fetch_assoc()) {
                                                    ...
                                                    $employee_id = $medical_data['employee_id'];

                                                    // ใช้ข้อมูลจาก result เป็นค่าหลัก
                                                    $latestEmployees[$employee_id] = $medical_data;
                                            ...
                                            // ดึงข้อมูลจาก result_medical_create (เฉพาะพนักงานที่ไม่มีใน result)
                                            if ($result_medical_create && $result_medical_create->num_rows > 0) {
                                                while ($row = $result_medical_create->fetch_assoc()) {
                                                    $medical_create = json_decode($row['medical'], true);
                                                    $employee_id = $medical_create['employee_id'];

                                                    // ถ้าพนักงานยังไม่มีข้อมูลใน result ให้ใช้จาก result_medical_create
                                                    if (!isset($latestEmployees[$employee_id])) {
                                                        $latestEmployees[$employee_id] = $medical_create;

                                                        if (isset($medical_create['price']) && is_numeric($medical_create['price'])) {
                                                            $totalPrice += $medical_create['price'];
                                                        }
                                                    }
                                                }
                                            }

                                            // เรียงข้อมูลตาม employee_id
                                            usort($latestEmployees, function ($a, $b) {
                                                return $a['employee_id'] <=> $b['employee_id'];
                                            });

                                            // แสดงข้อมูลในตาราง
                                            foreach ($latestEmployees as $medical_data) :
                                            ?>
                                    ...
                                    <!-- <tbody>
                                        <tr>
                                            ...
                                            if ($result_medical_create && $result_medical_create->num_rows > 0) :
                                                ...
                                                foreach ($latestEmployees as $medical_create) :
                                            ?>
                                                    <td scope="row"><?php echo $id++; ?></td>
                                                    <td><?php echo ($medical_create['year']); ?></td>
                                                    <td>
                                                        <a href="#" class="employee-link" data-employee="<?php echo ($medical_create['employee_name']); ?>">
                                                            <?php echo ($medical_create['employee_name']); ?>
                                                        </a>
                                                    </td>
                                                    <td><?php echo ($medical_create['employee_id']); ?></td>
                                                    <td><?php echo ($medical_create['department']); ?></td>
                                                    <td><?php echo ($medical_create['workage_year']); ?></td>
                                                    <td class="text-primary"><?php echo number_format($medical_create['amount'], 2); ?></td>
                                                    <td class="text-danger"><?php echo number_format($medical_create['value'], 2); ?></td>
                                                    <td class="text-success"><?php echo number_format($medical_create['balance'], 2); ?></td>
                                        </tr>
                                    <?php
                                                endforeach;
                                            else :
                                ...
                                ?>
                                    ...
                                    <!-- <tfoot>
                                        <tr>
                                            <td colspan="6" style="font-weight: bold; font-size: 16px;">รวมเป็นเงินทั้งสิ้น</td>
                                            <td></td>
    ...
    <script>
        ...
        //         // ส่งฟอร์มอัตโนมัติเมื่อมีการเปลี่ยนค่า
        ...
        document.getElementById('downloadExcel').addEventListener('click', function() {
            var table = document.getElementById('employee-details-table');
            var data = [];
